...
/// Write a single byte.
pub fn write_byte<W: Write>(w: &mut W, byte: u8) -> Result<usize> {
    w.write_all(&[byte])?;
    Ok(1)
}
...
///
/// Format: 1 sign byte (0=positive, 1=negative) + `int_size` magnitude bytes (LSB first).
pub fn write_int<W: Write>(w: &mut W, value: i32, int_size: u8) -> Result<usize> {
    let sign: u8 = if value < 0 { 1 } else { 0 };
    let mut written = write_byte(w, sign)?;
    let magnitude = value.unsigned_abs();
    for i in 0..int_size {
        written += write_byte(w, ((magnitude >> (i * 8)) & 0xFF) as u8)?;
    }
    Ok(written)
}
...
pub fn write_string<W: Write>(w: &mut W, value: Option<&str>, int_size: u8) -> Result<usize> {
    match value {
        None => write_int(w, -1, int_size),
        Some("") => write_int(w, 0, int_size),
        Some(s) => {
            let mut written = write_int(w, s.len() as i32, int_size)?;
...
pub fn write_offset<W: Write>(
    ...
    let mut written = write_byte(w, state as u8)?;
    for i in 0..off_size {
        written += write_byte(w, ((offset >> (i * 8)) & 0xFF) as u8)?;
    }
    Ok(written)
}
...
pub fn write_timestamp<W: Write>(w: &mut W, ts: &Timestamp, int_size: u8) -> Result<usize> {
    let mut written = 0;
    written += write_int(w, ts.second, int_size)?;
    written += write_int(w, ts.minute, int_size)?;
    written += write_int(w, ts.hour, int_size)?;
    written += write_int(w, ts.day, int_size)?;
    written += write_int(w, ts.month, int_size)?;
    written += write_int(w, ts.year, int_size)?;
    written += write_int(w, ts.is_dst, int_size)?;
    Ok(written)
}
...
mod tests {
    ...
    fn test_read_write_byte() {
        let mut buf = Vec::new();
        let written = write_byte(&mut buf, 0x42).unwrap();
        assert_eq!(written, 1);
    ...
    fn test_read_write_int_positive() {
        let mut buf = Vec::new();
        let written = write_int(&mut buf, 42, 4).unwrap();
        assert_eq!(written, 5);
    ...
    fn test_read_write_int_negative() {
        let mut buf = Vec::new();
        let written = write_int(&mut buf, -99, 4).unwrap();
        assert_eq!(written, 5);
    ...
    fn test_read_write_int_zero() {
        let mut buf = Vec::new();
        let written = write_int(&mut buf, 0, 4).unwrap();
        assert_eq!(written, 5);
    ...
    fn test_read_write_int_max() {
        let mut buf = Vec::new();
        let written = write_int(&mut buf, i32::MAX, 4).unwrap();
        assert_eq!(written, 5);
    ...
    fn test_read_write_int_min() {
        ...
        let mut buf = Vec::new();
        let written = write_int(&mut buf, i32::MIN, 4).unwrap();
        assert_eq!(written, 5);
    ...
    fn test_read_write_string_some() {
        let mut buf = Vec::new();
        let written = write_string(&mut buf, Some("hello"), 4).unwrap();
    ...
    fn test_read_write_string_none() {
        let mut buf = Vec::new();
        let written = write_string(&mut buf, None, 4).unwrap();
        assert_eq!(written, 5);
    ...
    fn test_read_write_string_empty() {
        let mut buf = Vec::new();
        let written = write_string(&mut buf, Some(""), 4).unwrap();
        assert_eq!(written, 5);
    ...
    fn test_read_write_string_unicode() {
        let mut buf = Vec::new();
        let value = "hello \u{1F600} world";
        let written = write_string(&mut buf, Some(value), 4).unwrap();
        assert_eq!(written, 5 + value.len());
        assert_eq!(
            read_string(&mut Cursor::new(&buf), 4).unwrap(),
    ...
    fn test_read_write_offset_set() {
        let mut buf = Vec::new();
        let written = write_offset(&mut buf, OffsetState::Set, 12345, 8).unwrap();
        assert_eq!(written, 9);
    ...
    fn test_read_write_offset_no_data() {
        let mut buf = Vec::new();
        let written = write_offset(&mut buf, OffsetState::NoData, 0, 8).unwrap();
        assert_eq!(written, 9);
        let (state, offset) = read_offset(&mut Cursor::new(&buf), 8).unwrap();
    ...
    fn test_read_write_offset_large() {
        ...
        let written = write_offset(&mut buf, OffsetState::Set, big_offset, 8).unwrap();
        assert_eq!(written, 9);
        let (state, offset) = read_offset(&mut Cursor::new(&buf), 8).unwrap();
        assert_eq!(state, OffsetState::Set);
    ...
    fn test_read_write_timestamp() {
        ...
        let mut buf = Vec::new();
        let written = write_timestamp(&mut buf, &ts, 4).unwrap();
    ...
    fn test_multiple_ints_sequential() {
        let mut buf = Vec::new();
        let written1 = write_int(&mut buf, 1, 4).unwrap();
        let written2 = write_int(&mut buf, 2, 4).unwrap();
        let written3 = write_int(&mut buf, -3, 4).unwrap();
        assert_eq!(written1 + written2 + written3, 15);
